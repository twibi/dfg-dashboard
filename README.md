...
All three filter groups sit on one line above the chart:
...
| **Основа** | Сите · Без полит. партии · Без полит. партии и спорт | swaps the total line and the `%` denominator; the per-institution rows are unaffected |
...
Missing years always render as a **gap** in the line (never as zero), and the
caption under the chart restates the current unit, basis and year range —
that caption is the only text below the chart.
...
- Percentages are shares of the selected basis's annual total, so the total
  line itself always reads 100 %.

## Design

- Main colour `#2f3283` (headings, level switch, total line) · accent
  `#ec7627` (action buttons, active selectors) · background `#e9e9e9` — the
  same family as the SELDI dashboard.
